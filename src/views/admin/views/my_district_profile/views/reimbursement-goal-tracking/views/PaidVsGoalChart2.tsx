...
const PaidVsGoalChart2: React.FC = () => {
  const chartRef = useRef<am5.Root | null>(null);

  const { apiValue, isLoading, error } = useApi();

  const paid: number = apiValue?.reimbursementGoalReports?.[0]?.lastYearPaidTotal || 0;
  const goal: number = apiValue?.reimbursementGoalReports?.[0]?.lastYearGoal || 0;

  useLayoutEffect(() => {
    const root: am5.Root = am5.Root.new("paidVsGoalChart2");
    root._logo?.dispose();
  ...
  return (
    <Box sx={{ width: "50%" }}>
      ...
      <div id="paidVsGoalChart2" style={{ width: "100%", height: "300px" }} />
    </Box>
  );
};

export default PaidVsGoalChart2;
